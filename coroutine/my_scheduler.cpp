...
void my_Scheduler::add_task(int socket)
{
    ...
    if (is_new_task) // 新创建的任务
    {
        ...
        // 新添加的任务需要绑定协程 tasklist中是等待绑定的任务
...
void my_Scheduler::read_task(my_Coroutine *coroutine)
{
    ...
    char *read_buf = new char[READ_BUFFER_SIZE];
...
void my_Scheduler::write_task(my_Coroutine *coroutine)
{
    ...
    while (bytes_sent < total_bytes)
    {
        ...
        if (ret < 0)
        {
            ...
            case EAGAIN:
                ...
                // L << "Socket " << task->get_socket() << " write buffer full, waiting for space" << endl;
                coroutine->yield();
                continue;
            case EBADF:
            case ECONNRESET:
            case EPIPE:
                ...
                break;
            default:
                LOG_ERROR("Unexpected error while sending data on socket %d: %s", task->get_socket(), strerror(errno));
                task->set_status(my_Task_node::ERROR);
                task->set_error_status(my_Task_node::WRITE_ERROR);
                break;
            }
        ...
        else
        {
            // 成功发送部分数据
            bytes_sent += ret;
            // LOG_INFO("Sent %d bytes, total %zu/%zu on socket %d",ret, bytes_sent, total_bytes, task->get_socket());
        }
    }
    ...
    else
    {
        ...
        LOG_ERROR("faile to send entire response to socket %d", task->get_socket());
    }
}
...
void my_Scheduler::close_task(my_Coroutine *coroutine)
{
    ...
    {
        std::lock_guard<std::mutex> lock_socket_to_coroutine(my_socket_to_coroutine_mutex);
        my_socket_to_coroutine_map.erase(task->get_socket());
        if (task->get_socket() != -1)
        {
...
void my_Scheduler::run()
{
    ...
    main_coroutine->init(1024 * 1024 * 8, nullptr);
    main_coroutine->set_main(main_coroutine);

    my_Coroutine *idle_coroutine = new my_Coroutine();
    idle_coroutine->init(1024 * 1024 * 8, std::bind(&my_Scheduler::idle, this));
    ...
    while (!stop_scheduler)
    {
        ...
        if (coroutine_run != nullptr && coroutine_run->get_task() != nullptr)
        {
            cout << "coroutine_run id:" << coroutine_run->get_id() << endl;
            cout << "coroutine_run task:" << coroutine_run->get_task()->get_socket() << endl;
            LOG_INFO("coroutine_run id:%d",coroutine_run->get_id());
            LOG_INFO("coroutine_run task:%d",coroutine_run->get_task()->get_socket());
            ...
            if (coroutine_run->get_task()->get_status() == my_Task_node::READ)
            {
                modfd(coroutine_run->get_task()->get_socket(), my_Task_node::READ); // 修改为可读事件
            }
            if (coroutine_run->get_task()->get_status() == my_Task_node::WRITE)
            {
                modfd(coroutine_run->get_task()->get_socket(), my_Task_node::WRITE); // 修改为可读事件
            }
            continue;
        }
        ...
        while (my_close_coroutine_num > 0)
        {
            ...
            if (coroutine)
            {
                if (is_coroutine_pool == true)
                {
                    coroutine->reset([this](my_Coroutine *coroutine)
                                     { Coroutine_run(coroutine, this); });
